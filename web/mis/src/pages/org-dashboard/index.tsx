...
import { getOrgDashboardUrl } from "@/services/metabse.service";
...
const OrgDashBoardPage = () => {
  ...
  const fetchLabDashUrl = async () => {
    showLoading();
    try {
      const res = await getOrgDashboardUrl(orgId);
      if (!res.error) {
        setState(`${res.data}`);
      }
    } catch (error) {
  ...
  if (!isOrgAdmin) {
    return <Navigate to={`/${orgId}/${routeConstants.LABS}`} />;
  }
  return (
    <>
      <SiteHeader title="Organization Dashboard" />
