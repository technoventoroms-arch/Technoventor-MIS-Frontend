...
import {
  CreateNewMachineFormType,
  default as MachineForm,
} from "./components/machine-form";
...
const ManageLabUsersPage = () => {
  ...
  const [deleteMachineModal, setDeleteMachineModal] = useState<{
  ...
  const table = useReactTable({
    ...
    meta: {
      ...
      deleteMachine: (item: IMachine) =>
        setDeleteMachineModal({ content: item, open: true }),
  ...
  const handleCreateNewMachine = async (data: CreateNewMachineFormType) => {
    setLoading(true);
    try {
      const res = await createNewMachine({
        ...data,
      });
      if (!res.error) {
        setMachines({ ...machine, records: [res.data, ...machine.records] });
        setCreateMachineModal(false);
        toast.success("Successfully created machine.");
      }
    } catch (error: any) {
      toast.error(getAxiosErrorMessage(error));
    } finally {
  ...
  const handleDeleteMachine = async () => {
    setLoading(true);
    try {
      const res = await deleteMachine(deleteMachineModal.content!.id!);
      if (!res.error) {
        setDeleteMachineModal({ content: null, open: false });
        await getMachines({
          ...filters,
          status: filters.status != "ALL" ? filters.status : "",
          skip: 0,
  ...
  return (
    <>
          ...
          extraTableContent={
            <>
              <div className="ml-auto ">
                ...
                <CanIUse action={(a) => a.CREATE_MACHINES}>
                  <Button
                    className=""
                    variant="green"
                    size="sm"
                  ...
                  >
                    <PlusIcon />
                    <span className="hidden lg:inline">Add New Machine</span>
                  </Button>
                </CanIUse>
              </div>
            </>
      ...
      <ResponsiveDrawer
        open={!!createMachineModal}
        onOpenChange={() => {
          !loading && setCreateMachineModal(false);
        }}
      >
        <DrawerContent className="data-[vaul-drawer-direction=bottom]:max-h-[90vh] data-[vaul-drawer-direction=bottom]:mt-4!">
          <ResponsiveDrawer.Header
            ...
            description="Modal for creating new machine."
          />
          <MachineForm
            loading={loading}
            handleSubmit={handleCreateNewMachine}
          />
        </DrawerContent>
      </ResponsiveDrawer>
      ...
      <ResponsiveDrawer
        ...
        onAnimationEnd={() => {
          !loading && setShowMachineQRModal({ open: false, content: null });
        }}
      >
        <DrawerContent className="data-[vaul-drawer-direction=bottom]:max-h-[90vh] data-[vaul-drawer-direction=bottom]:mt-4!">
          <ResponsiveDrawer.Header
      ...
      <GenericModal
        desc={
          <>
            ...
            <Badge variant={"red"}>{deleteMachineModal.content?.name}</Badge>
          </>
        }
        onConfirmClick={handleDeleteMachine}
        onOpenChange={(e) => setDeleteMachineModal({ content: null, open: e })}
        open={deleteMachineModal.open}
